...
Both RSCs and Astro components adopt a server-only mental model, which enables the usage of "top-level await" for data fetching on the server side. The fetched data can then be directly serialized and passed to client components as props, integrating data fetching within the component tree itself.
