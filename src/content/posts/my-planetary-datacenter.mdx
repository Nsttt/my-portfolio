...
I jokingly refer to my setup as a **_“Planetary Datacenter”_** because it stretches beyond just my house. Thanks to _clever networking_ and tools like **ZeroTier**, I can place services just about anywhere on the globe (hence **_“planetary”_**) and still have them communicate as if they were right next to each other on my local LAN. If you’re into homelabs, networking, or just curious about how to structure your own multi-machine environment, _stick around_.

I’ll walk you through all of it.

## What is the _“Planetary Datacenter”_ anyway?

I first introduced this idea at a **CNCF Galicia** talk, where I discussed how I use **ZeroTier** and **WireGuard** to interconnect various machines. Over time, I realized that combining ZeroTier with my local hardware gave me a pretty powerful setup: everything in one _“big” virtual LAN_, accessible anywhere.
...
- **Planet-Wide Access:** I can spin up a new service on a VPS, at home, or in a cloud provider. As soon as I hook it up to my **ZeroTier** network, it feels like it’s right there on my LAN. And in a worst case scenario, migrating can be done with a _click of a button_.
...
My ISP router is basically acting as a **pass-through device**. I’ve set it up with DMZ mode pointing straight to my **_UDM Pro Max_** (a Unifi Dream Machine Pro Max). The UDM is the _nerve center_ of my home network. It handles VLANs, firewall rules, and all the fancy Unifi-based management. If you’re familiar with the Unifi ecosystem, you know it’s pretty user-friendly yet powerful enough to do some advanced stuff if you know how to hack it enough.
...
1. **NAS (TrueNAS Scale)**
   ...
   - **Role**: Holds all my data, shared mainly over _NFSv4.2_. I also install the **ZeroTier** app directly on it so it has a presence on my _global_ network.
...
1. **LAN (VLAN 1)**: The _default network_ for my local machines and most VMs.
...
This bridging is done via some _custom scripts_ and the UDM’s “persistence” system. It’s a bit of a hack but allows ZeroTier traffic to flow in and out of my local VLANs as if they were physically plugged in.
...
To make management easy, I use a _community-built UI_ that gives me a web dashboard for seeing which devices are connected, assigning IP addresses, and approving or denying new join requests. It’s _containerized_ using the same MacVLAN trick I mentioned, so it’s directly available on VLAN 2 without any weird network hoops.
...
- **Microcode Patches**: If you’re using a _new-gen_ Intel CPU, you might need microcode patches to avoid weird performance or stability issues.

- **GPU Passthrough**: Proxmox makes it pretty straightforward. You just enable _Intel IOMMU_ in the grub config, load the vfio modules, and pass the GPU through to whichever VM needs it.

- **VMs**: I run four main VMs. Portainer, Network, Media, and Misc.

## VM setup

One of the **key pieces** of this setup is a _MacVLAN_ approach that allows my containers to attach _directly_ to the ZeroTier VLAN interfaces.  
It keeps _host networking_ simpler and gives each container a dedicated IP isolated to the ZeroTier VLAN.
...
- This is my _entertainment hub_. It has the full \*Arr suite (**Sonarr**, **Radarr**, **Bazarr**, **Lidarr**) plus **Jellyfin** for streaming, **SABnzbd**, **qBittorrent**, and a **Gluetun** container for VPN.
...
You just have to be consistent with your paths in **Docker Compose** and your \*Arr apps, so everything references the same _“root”_ mount point.
...
While **LXC** can be more lightweight and can potentially yield _better performance_, it’s just trickier to _replicate_ consistently.
